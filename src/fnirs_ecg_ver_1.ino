...
#define ECG_BATCH       48     //ECG samples buffered between pushes (26 at 128 sps / 200 ms)
#define ECG_DRAIN       8      //max ECG FIFO reads per frame
...
Protocol: every websocket message is a 3-character tag followed by its payload.
    ...
    rri<ms,ms,...>    accepted R-R intervals [ms]
*/
...
var NWIN = WIN_S * 50, EWIN = WIN_S * 128, LOGCAP = 400000;
...
function onMsg(ev){
  ...
  if (tag == 'hrv'){ $('hrv').textContent = v; }
    ...
    for (var k = 0; k < r.length; k++){
      var ms = parseInt(r[k], 10);
      if (logR.length < LOGCAP) logR.push([(nIdx / 50).toFixed(2), ms]);
...
void loop() {
    ...
    if (rr <= 1500 && rr >= 500) { //Check if data is noise
        if (cRrInt != rr) { //Check if the data is the same as a previous sample. Likelihood is high, as the sampling rate is faster than the heartbeat
            ...
            if (rrCount < RR_BATCH) rrBuf[rrCount++] = rr;
        }
    }

    //Drain whatever the ECG FIFO has accumulated since the last frame
    //(128 sps against a 50 Hz frame rate is ~2.6 samples).
    int32_t ecgSample;
